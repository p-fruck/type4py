...
from type4py.utils import load_model_params, create_tmp_file
...
def infer_main(pre_trained_model_path: str, source_file_path: str, check_types: bool):
    """
    Infer types for a given source file locally and typecheck them.

    This method is invoked using the `type4py infer` command.
    """
    ...
    t4py_pretrained_m = PretrainedType4Py(pre_trained_model_path, "cpu", False, True)
    t4py_pretrained_m.load_pretrained_model()

    print(src_f_read, flush=True)
    annotated_unchecked = type_annotate_file(t4py_pretrained_m, src_f_read.encode())
    print("=== Predicted variable types:", annotated_unchecked["variables_p"], flush=True)
    if check_types:
        annotated_checked = get_type_checked_preds(annotated_unchecked, src_f_read)
        print("=== Validated variable types", annotated_checked["variables_p"], flush=True)

    # is_tc, p_type = type_check_pred(source_file_path, src_f_o_path, src_f_ext, mypy_tc, p, m_v_t)
...
def type_check_pred(
    src_f_r: str, src_f_o_path: str, src_f_ext: dict, tc: MypyManager, pred: str, true: str
) -> Tuple[bool, PredictionType]:
    """
    Type checks a prediction and returns validation result (<true/false>, PredictionType)
    """
...
def get_type_checked_preds(src_f_ext: dict, src_f_read: str) -> dict:
    """Called from the API server. Use static type checking to only returned validated type predictions."""
